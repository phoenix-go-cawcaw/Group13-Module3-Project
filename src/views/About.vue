...
// ── Colour tokens (Warm Craft Palette — matches HomeView) ──────────
// Cream Canvas    #F3E6D3   Warm Sand      #E2C49A   Caramel Beige  #D1A877
// Burnt Caramel   #C06A22   Deep Amber     #A95A1C
// Rich Cocoa      #5A3A22   Dark Espresso  #3E2A1B
...
<template>
  <main class="about-root">
    ...
    <!-- Shared background layers (matches HomeView) -->
    <div class="bg-wash" />
    <div class="bg-grain" />

    <!-- Floating decorative bg icons -->
    ...
    <section class="section hero-section">
      <div class="hero-grid">
        ...
        <div ref="heroText" class="hero-text anim-fade-left">
          ...
          <p class="hero-sub">
            <strong>Hobby in a Box</strong> delivers curated hobby experiences every month, so you can explore your passions, learn new skills, and have fun from home.
          </p>
        ...
        <div class="features-grid">
          ...
          >
            ...
            <div class="feat-icon-wrap">
              <!-- Package -->
              <svg v-if="f.icon==='package'" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                <path d="M16.5 9.4l-9-5.19M21 16V8a2 2 0 00-1-1.73l-7-4a2 2 0 00-2 0l-7 4A2 2 0 003 8v8a2 2 0 001 1.73l7 4a2 2 0 002 0l7-4A2 2 0 0021 16z"/>
                <polyline points="3.27 6.96 12 12.01 20.73 6.96"/><line x1="12" y1="22.08" x2="12" y2="12"/>
              </svg>
              <!-- Refresh -->
              <svg v-else-if="f.icon==='refresh'" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                <polyline points="23 4 23 10 17 10"/><polyline points="1 20 1 14 7 14"/>
                <path d="M3.51 9a9 9 0 0114.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0020.49 15"/>
              </svg>
              <!-- Grad cap -->
              <svg v-else-if="f.icon==='grad'" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                <path d="M22 10v6M2 10l10-5 10 5-10 5z"/>
                <path d="M6 12v5c3 3 9 3 12 0v-5"/>
              </svg>
              <!-- Heart -->
              <svg v-else-if="f.icon==='heart'" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        ...
        <div class="categories-grid">
          ...
          >
            ...
            <div class="cat-content">
              <div class="cat-icon">
                <!-- Palette -->
                <svg v-if="cat.icon==='palette'" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
                  <circle cx="13.5" cy="6.5" r=".5" fill="currentColor"/><circle cx="17.5" cy="10.5" r=".5" fill="currentColor"/>
                  <circle cx="8.5" cy="7.5" r=".5" fill="currentColor"/><circle cx="6.5" cy="12.5" r=".5" fill="currentColor"/>
                  <path d="M12 2C6.5 2 2 6.5 2 12s4.5 10 10 10c.926 0 1.648-.746 1.648-1.688 0-.437-.18-.835-.437-1.125-.29-.289-.438-.652-.438-1.125a1.64 1.64 0 011.668-1.668h1.996c3.051 0 5.555-2.503 5.555-5.554C21.965 6.012 17.461 2 12 2z"/>
                </svg>
                <!-- Leaf -->
                <svg v-else-if="cat.icon==='leaf'" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                  <path d="M11 20A7 7 0 014 13V6a1 1 0 011-1h6a7 7 0 017 7v1a7 7 0 01-7 7z"/><path d="M11 20v-7M4 7l7 6"/>
                </svg>
                <!-- Flask -->
                <svg v-else-if="cat.icon==='flask'" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                  <path d="M10 2v7.31l-5.48 8.38A1 1 0 005.4 19h13.2a1 1 0 00.88-1.31L14 9.31V2"/>
                  <line x1="8.5" y1="2" x2="15.5" y2="2"/>
                </svg>
                <!-- Utensils -->
                <svg v-else-if="cat.icon==='utensils'" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
    ...
    <section class="section footer-section">
      <div ref="footerLogo" class="footer-logo anim-scale-up">
        <div class="logo-circle">
          <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#F3E6D3" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            <path d="M16.5 9.4l-9-5.19M21 16V8a2 2 0 00-1-1.73l-7-4a2 2 0 00-2 0l-7 4A2 2 0 003 8v8a2 2 0 001 1.73l7 4a2 2 0 002 0l7-4A2 2 0 0021 16z"/>
            <polyline points="3.27 6.96 12 12.01 20.73 6.96"/><line x1="12" y1="22.08" x2="12" y2="12"/>
          </svg>
        </div>
...
/* Floating deco — same as HomeView */
.deco-layer { position: fixed; inset: 0; pointer-events: none; overflow: hidden; z-index: 1; }
.deco       { position: absolute; color: rgba(192,106,34,0.14); }
.deco-palette { top: 4%;    left: 2%; }
.deco-sprout  { bottom: 6%; right: 3%; }
...
.anim-fade-right.visible-right { opacity: 1; transform: none; }
...
.hero-title {
  ...
  color: #5A3A22; margin: 0 0 1.2rem;
}
.hero-accent {
...
.hero-sub {
  font-size: 1.15rem; color: #5A3A22; line-height: 1.75; margin: 0 0 2rem;
}

/* CTA Button — matches HomeView btn-primary */
.btn-primary {
  display: inline-flex; align-items: center; gap: 8px;
  padding: .9rem 1.8rem; font-size: 1rem; font-weight: 700;
  border-radius: 14px; text-decoration: none; cursor: pointer;
  font-family: inherit;
  background: #C06A22; color: #F3E6D3;
  border: 2px solid #A95A1C; box-shadow: 0 6px 0 #A95A1C;
  transition: transform .15s ease, box-shadow .15s ease;
}
.btn-primary:hover  { transform: translateY(3px); box-shadow: 0 3px 0 #A95A1C; color: #F3E6D3; }
.btn-primary:active { transform: translateY(5px); box-shadow: 0 1px 0 #A95A1C; }
...
.hero-img { width: 100%; height: auto; display: block; max-height: 440px; object-fit: cover; }
...
.mission-body {
  font-size: 1.15rem; line-height: 1.8; color: #3E2A1B;
  margin: 0;
}

.section-title {
  font-size: clamp(1.8rem, 4vw, 3rem);
  font-weight: 900; color: #5A3A22;
...
.feature-card:hover { box-shadow: 0 16px 40px rgba(62,42,27,.16); transform: translateY(-4px); }

.feat-icon-wrap {
  width: 72px; height: 72px; border-radius: 50%;
  background: rgba(255,255,255,.5);
  display: flex; align-items: center; justify-content: center;
  color: #C06A22; margin-bottom: 1.2rem;
}
.feat-title { font-size: 1.05rem; font-weight: 700; color: #3E2A1B; margin: 0 0 .5rem; }
.feat-desc  { font-size: .88rem; color: #5A3A22; line-height: 1.55; margin: 0; }

.categories-grid {
  display: grid; grid-template-columns: 1fr; gap: 1.5rem;
}
@media (min-width: 700px) { .categories-grid { grid-template-columns: 1fr 1fr; } }
...
.cat-card:hover { transform: translateY(-6px); box-shadow: 0 24px 48px rgba(62,42,27,.25); }
...
.cat-icon { color: #E2C49A; margin-bottom: .5rem; }
.cat-title    { font-size: 1.55rem; font-weight: 900; margin: 0 0 .3rem; }
.cat-subtitle { font-size: .9rem; color: #E2C49A; font-weight: 600; margin: 0; }

.footer-section { padding: 4rem 1.5rem 5rem; text-align: center; }
...
.footer-brand {
  ...
  color: #5A3A22; letter-spacing: -0.02em; margin: 0;
}
.footer-copy { font-size: .85rem; color: #5A3A22; opacity: .6; margin: 0; }
</style>